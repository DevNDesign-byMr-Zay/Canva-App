...
export function buildHolographicCanvaPayload({
  scene,
  ...
  const payload = deepFreeze(
    cloneValue({
      ...
      authoritativeSource:
        value.rendererContract?.authoritativeSource ?? 'thergrid-decision-receipt',
      target,
      snapshotId,
      ...
      attention: attentionItems.map((item) => ({
        ...
        evidenceRef:
          item.evidenceRef == null ? null : text(item.evidenceRef, 'attention.evidenceRef'),
...
function fingerprintInput(payload) {
  const { payloadFingerprint: _payloadFingerprint, safety: _safety, ...unsignedPayload } = payload;
...
export function validateHolographicCanvaPayload(payload) {
  try {
    ...
    return (
      ...
      value.sceneIdentity.trim().length > 0 &&
      typeof value.provenanceRef === 'string' &&
      value.provenanceRef.trim().length > 0 &&
