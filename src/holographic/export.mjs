...
export function exportHolographicScene(
  scene,
  { format = 'json', target = 'simulator', displayProfile = 'default' } = {},
) {
  if (!scene || scene.schema !== 'holo.scene.v1') {
    throw new TypeError('A holo.scene.v1 scene is required.');
  }
  if (format !== 'json') {
    throw new TypeError(`Unsupported holographic export format: ${format}`);
  }
