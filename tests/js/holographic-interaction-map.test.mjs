...
test('maps editable Canva interactions into advisory holographic targets', () => {
  const payload = compileAiHolographicScene({
    snapshotId: 'snap-1', provenanceRef,
    modelOutput: { intentVersion: 1, target: 'holo-mat', nodes: [{ id: 'node-1', canvaElementId: 'el-7', role: 'alert', text: 'Inspect', interaction: { action: 'inspect' }, position: { x: 1, y: 2, z: 3 } }] },
  });
...
test('rejects unsupported interaction actions', () => {
  const payload = compileAiHolographicScene({
    snapshotId: 'snap-2', provenanceRef,
    modelOutput: { intentVersion: 1, target: 'holo-mat', nodes: [{ id: 'node-1', text: 'X', interaction: { action: 'launch' }, position: { x: 0, y: 0, z: 0 } }] },
  });
  assert.throws(() => mapCanvaHolographicInteractions(payload), /unsupported interaction action/);
});
