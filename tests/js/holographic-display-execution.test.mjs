...
describe('holographic display execution planning', () => {
  ...
  for (const [displayProfile, target, operation, expectedPolicy] of cases) {
    test(`maps ${displayProfile} profile to an explicit operation and render policy`, () => {
      ...
      assert.deepEqual({ ...plan.renderPolicy, ...expectedPolicy }, plan.renderPolicy);
  ...
  test('rejects a profile when the physical target type does not match', () => {
    assert.throws(
      () => createDisplayExecutionPlan({ scene, target: 'holomat-1', displayProfile: 'projector' }),
      /incompatible/,
    );
  });
});
