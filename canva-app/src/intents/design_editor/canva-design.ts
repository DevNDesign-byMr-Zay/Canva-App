...
type HoloForgeScenario = {
  scenarioId: string;
  source: { designId: string; snapshotId: string; snapshotFingerprint: string };
  candidate: { changedElementIds: string[]; layout: { elements?: CandidateElementTransform[] } };
  evidence: { status: string; hardConstraintsPassed: boolean };
  presentation: { advisoryOnly: boolean; autoApply: boolean; target: string };
  provenance: { scenarioFingerprint: string; optimizationFingerprint: string };
};
...
export async function readCurrentDesignSnapshot(): Promise<CanvaDesignSnapshot> {
  ...
  await openDesign({ type: "current_page" }, async (session) => {
    ...
    elements = session.page.elements.toArray().map((element) => ({
      id: element.id, type: element.type, top: element.top, left: element.left,
      width: element.width, height: element.height, rotation: element.rotation, locked: element.locked,
    }));
  });
...
export function canApplyScenario(scenario: HoloForgeScenario | null | undefined, snapshot: CanvaDesignSnapshot | null | undefined): boolean {
  ...
  if (!Array.isArray(transforms) || transforms.length === 0 || !Array.isArray(scenario.candidate.changedElementIds) || scenario.candidate.changedElementIds.length === 0) return false;
  const knownIds = new Set(snapshot.elements.map(({ id }) => id));
  return transforms.every(({ elementId, top, left, width, height, rotation }) => knownIds.has(elementId) && scenario.candidate.changedElementIds.includes(elementId) && [top, left, width, height, rotation].every((value) => value === undefined || Number.isFinite(value)));
}

export async function applyScenario(scenario: HoloForgeScenario, snapshot: CanvaDesignSnapshot): Promise<{ scenarioId: string; changedElementIds: string[] }> {
  if (!canApplyScenario(scenario, snapshot)) throw new Error("Scenario is not safe to apply: it is stale, incomplete, unsupported, or unverified.");
  const transforms = scenario.candidate.layout.elements!;

  await openDesign({ type: "current_page" }, async (session) => {
    if (session.page.type !== "absolute" || session.page.locked || session.page.id !== snapshot.pageId) throw new Error("The Canva page is no longer compatible with the selected scenario.");
    ...
    for (const transform of transforms) {
      ...
      if (!element || element.locked || element.type === "unsupported") throw new Error(`Scenario references an unavailable element: ${transform.elementId}`);
  ...
  return { scenarioId: scenario.scenarioId, changedElementIds: transforms.map(({ elementId }) => elementId) };
}
